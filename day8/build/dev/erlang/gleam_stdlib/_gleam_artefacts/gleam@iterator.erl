...
-type action(BQT) :: stop | {continue, BQT, fun(() -> action(BQT))}.

-opaque iterator(BQU) :: {iterator, fun(() -> action(BQU))}.

-type step(BQV, BQW) :: {next, BQV, BQW} | done.

-type chunk(BQX, BQY) :: {another_by,
        list(BQX),
        BQY,
        BQX,
        fun(() -> action(BQX))} |
...
-type sized_chunk(BQZ) :: {another, list(BQZ), fun(() -> action(BQZ))} |
    {last, list(BQZ)} |
    no_more.
...
-spec do_unfold(BRC, fun((BRC) -> step(BRD, BRC))) -> fun(() -> action(BRD)).
do_unfold(Initial, F) ->
...
-spec unfold(BRH, fun((BRH) -> step(BRI, BRH))) -> iterator(BRI).
unfold(Initial, F) ->
    _pipe = Initial,
    _pipe@1 = do_unfold(_pipe, F),
    {iterator, _pipe@1}.

-spec repeatedly(fun(() -> BRM)) -> iterator(BRM).
repeatedly(F) ->
    unfold(nil, fun(_) -> {next, F(), nil} end).

-spec repeat(BRO) -> iterator(BRO).
repeat(X) ->
    repeatedly(fun() -> X end).

-spec from_list(list(BRQ)) -> iterator(BRQ).
from_list(List) ->
...
-spec transform(iterator(BSA), BSC, fun((BSC, BSA) -> step(BSD, BSC))) -> iterator(BSD).
transform(Iterator, Initial, F) ->
    _pipe = do_transform(erlang:element(2, Iterator), Initial, F),
    {iterator, _pipe}.

-spec do_fold(fun(() -> action(BSH)), fun((BSJ, BSH) -> BSJ), BSJ) -> BSJ.
...
-spec fold(iterator(BSK), BSM, fun((BSM, BSK) -> BSM)) -> BSM.
...
-spec to_list(iterator(BSP)) -> list(BSP).
to_list(Iterator) ->
    _pipe = Iterator,
    _pipe@1 = fold(_pipe, [], fun(Acc, E) -> [E | Acc] end),
    gleam@list:reverse(_pipe@1).

-spec step(iterator(BSS)) -> step(BSS, iterator(BSS)).
step(Iterator) ->
...
-spec do_take(fun(() -> action(BSX)), integer()) -> fun(() -> action(BSX)).
...
-spec take(iterator(BTA), integer()) -> iterator(BTA).
take(Iterator, Desired) ->
    _pipe = erlang:element(2, Iterator),
    _pipe@1 = do_take(_pipe, Desired),
    {iterator, _pipe@1}.

-spec do_drop(fun(() -> action(BTD)), integer()) -> action(BTD).
...
-spec drop(iterator(BTG), integer()) -> iterator(BTG).
drop(Iterator, Desired) ->
    _pipe = fun() -> do_drop(erlang:element(2, Iterator), Desired) end,
    {iterator, _pipe}.

-spec do_map(fun(() -> action(BTJ)), fun((BTJ) -> BTL)) -> fun(() -> action(BTL)).
...
-spec map(iterator(BTN), fun((BTN) -> BTP)) -> iterator(BTP).
map(Iterator, F) ->
    _pipe = erlang:element(2, Iterator),
...
-spec map2(iterator(BTX), iterator(BTZ), fun((BTX, BTZ) -> BUB)) -> iterator(BUB).
...
-spec do_append(fun(() -> action(BUD)), fun(() -> action(BUD))) -> action(BUD).
...
-spec append(iterator(BUH), iterator(BUH)) -> iterator(BUH).
append(First, Second) ->
    _pipe = fun() ->
...
-spec do_flatten(fun(() -> action(iterator(BUL)))) -> action(BUL).
...
-spec flatten(iterator(iterator(BUP))) -> iterator(BUP).
flatten(Iterator) ->
    _pipe = fun() -> do_flatten(erlang:element(2, Iterator)) end,
    {iterator, _pipe}.

-spec concat(list(iterator(BUT))) -> iterator(BUT).
concat(Iterators) ->
    flatten(from_list(Iterators)).

-spec flat_map(iterator(BUX), fun((BUX) -> iterator(BUZ))) -> iterator(BUZ).
flat_map(Iterator, F) ->
    _pipe = Iterator,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).

-spec do_filter(fun(() -> action(BVC)), fun((BVC) -> boolean())) -> action(BVC).
...
-spec filter(iterator(BVF), fun((BVF) -> boolean())) -> iterator(BVF).
filter(Iterator, Predicate) ->
    _pipe = fun() -> do_filter(erlang:element(2, Iterator), Predicate) end,
    {iterator, _pipe}.

-spec cycle(iterator(BVI)) -> iterator(BVI).
cycle(Iterator) ->
    _pipe = repeat(Iterator),
    flatten(_pipe).

-spec do_find(fun(() -> action(BVM)), fun((BVM) -> boolean())) -> {ok, BVM} |
    {error, nil}.
...
-spec find(iterator(BVQ), fun((BVQ) -> boolean())) -> {ok, BVQ} | {error, nil}.
find(Haystack, Is_desired) ->
    _pipe = erlang:element(2, Haystack),
    do_find(_pipe, Is_desired).

-spec do_index(fun(() -> action(BVU)), integer()) -> fun(() -> action({integer(),
    BVU})).
...
-spec index(iterator(BVX)) -> iterator({integer(), BVX}).
index(Iterator) ->
    _pipe = erlang:element(2, Iterator),
    _pipe@1 = do_index(_pipe, 0),
    {iterator, _pipe@1}.

-spec iterate(BWA, fun((BWA) -> BWA)) -> iterator(BWA).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-spec do_take_while(fun(() -> action(BWC)), fun((BWC) -> boolean())) -> fun(() -> action(BWC)).
...
-spec take_while(iterator(BWF), fun((BWF) -> boolean())) -> iterator(BWF).
take_while(Iterator, Predicate) ->
    _pipe = erlang:element(2, Iterator),
    _pipe@1 = do_take_while(_pipe, Predicate),
    {iterator, _pipe@1}.

-spec do_drop_while(fun(() -> action(BWI)), fun((BWI) -> boolean())) -> action(BWI).
...
-spec drop_while(iterator(BWL), fun((BWL) -> boolean())) -> iterator(BWL).
drop_while(Iterator, Predicate) ->
    _pipe = fun() -> do_drop_while(erlang:element(2, Iterator), Predicate) end,
    {iterator, _pipe}.

-spec do_scan(fun(() -> action(BWO)), fun((BWQ, BWO) -> BWQ), BWQ) -> fun(() -> action(BWQ)).
...
-spec scan(iterator(BWS), BWU, fun((BWU, BWS) -> BWU)) -> iterator(BWU).
scan(Iterator, Initial, F) ->
    _pipe = erlang:element(2, Iterator),
    _pipe@1 = do_scan(_pipe, F, Initial),
    {iterator, _pipe@1}.

-spec do_zip(fun(() -> action(BWW)), fun(() -> action(BWY))) -> fun(() -> action({BWW,
    BWY})).
do_zip(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(iterator(BXB), iterator(BXD)) -> iterator({BXB, BXD}).
zip(Left, Right) ->
    _pipe = do_zip(erlang:element(2, Left), erlang:element(2, Right)),
    {iterator, _pipe}.

-spec next_chunk(fun(() -> action(BXG)), fun((BXG) -> BXI), BXI, list(BXG)) -> chunk(BXG, BXI).
...
-spec do_chunk(fun(() -> action(BXM)), fun((BXM) -> BXO), BXO, BXM) -> action(list(BXM)).
...
-spec chunk(iterator(BXR), fun((BXR) -> any())) -> iterator(list(BXR)).
chunk(Iterator, F) ->
...
-spec next_sized_chunk(fun(() -> action(BXW)), integer(), list(BXW)) -> sized_chunk(BXW).
...
-spec do_sized_chunk(fun(() -> action(BYA)), integer()) -> fun(() -> action(list(BYA))).
...
-spec sized_chunk(iterator(BYE), integer()) -> iterator(list(BYE)).
sized_chunk(Iterator, Count) ->
    _pipe = erlang:element(2, Iterator),
    _pipe@1 = do_sized_chunk(_pipe, Count),
    {iterator, _pipe@1}.

-spec do_intersperse(fun(() -> action(BYI)), BYI) -> action(BYI).
...
-spec intersperse(iterator(BYL), BYL) -> iterator(BYL).
...
-spec do_any(fun(() -> action(BYO)), fun((BYO) -> boolean())) -> boolean().
...
-spec any(iterator(BYQ), fun((BYQ) -> boolean())) -> boolean().
any(Iterator, Predicate) ->
    _pipe = erlang:element(2, Iterator),
    do_any(_pipe, Predicate).

-spec do_all(fun(() -> action(BYS)), fun((BYS) -> boolean())) -> boolean().
...
-spec all(iterator(BYU), fun((BYU) -> boolean())) -> boolean().
all(Iterator, Predicate) ->
    _pipe = erlang:element(2, Iterator),
    do_all(_pipe, Predicate).

-spec update_group_with(BYW) -> fun((gleam@option:option(list(BYW))) -> list(BYW)).
...
-spec group_updater(fun((BZA) -> BZB)) -> fun((gleam@dict:dict(BZB, list(BZA)), BZA) -> gleam@dict:dict(BZB, list(BZA))).
group_updater(F) ->
    fun(Groups, Elem) -> _pipe = Groups,
        gleam@dict:update(_pipe, F(Elem), update_group_with(Elem)) end.

-spec group(iterator(BZI), fun((BZI) -> BZK)) -> gleam@dict:dict(BZK, list(BZI)).
group(Iterator, Key) ->
    _pipe = Iterator,
...
-spec reduce(iterator(BZO), fun((BZO, BZO) -> BZO)) -> {ok, BZO} | {error, nil}.
reduce(Iterator, F) ->
    case (erlang:element(2, Iterator))() of
        stop ->
            {error, nil};
...
-spec last(iterator(BZS)) -> {ok, BZS} | {error, nil}.
last(Iterator) ->
    _pipe = Iterator,
...
-spec once(fun(() -> BZY)) -> iterator(BZY).
once(F) ->
...
-spec single(CAA) -> iterator(CAA).
single(Elem) ->
    once(fun() -> Elem end).

-spec do_interleave(fun(() -> action(CAC)), fun(() -> action(CAC))) -> action(CAC).
...
-spec interleave(iterator(CAG), iterator(CAG)) -> iterator(CAG).
...
-spec do_fold_until(
    fun(() -> action(CAK)),
    fun((CAM, CAK) -> gleam@list:continue_or_stop(CAM)),
    CAM
) -> CAM.
...
-spec fold_until(
    iterator(CAO),
    CAQ,
    fun((CAQ, CAO) -> gleam@list:continue_or_stop(CAQ))
) -> CAQ.
...
-spec try_fold(iterator(CBA), CBC, fun((CBC, CBA) -> {ok, CBC} | {error, CBD})) -> {ok,
        CBC} |
    {error, CBD}.
...
-spec first(iterator(CBI)) -> {ok, CBI} | {error, nil}.
first(Iterator) ->
    case (erlang:element(2, Iterator))() of
        stop ->
            {error, nil};
...
-spec at(iterator(CBM), integer()) -> {ok, CBM} | {error, nil}.
at(Iterator, Index) ->
    _pipe = Iterator,
...
-spec each(iterator(CBU), fun((CBU) -> any())) -> nil.
each(Iterator, F) ->
    _pipe = Iterator,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).

-spec yield(CBX, fun(() -> iterator(CBX))) -> iterator(CBX).
yield(Element, Next) ->
