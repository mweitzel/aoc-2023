...
-opaque queue(EKV) :: {queue, list(EKV), list(EKV)}.

-spec new() -> queue(any()).
new() ->
    {queue, [], []}.

-spec from_list(list(EKY)) -> queue(EKY).
from_list(List) ->
    {queue, [], List}.

-spec to_list(queue(ELB)) -> list(ELB).
to_list(Queue) ->
...
-spec pop_front(queue(ELT)) -> {ok, {ELT, queue(ELT)}} | {error, nil}.
pop_front(Queue) ->
    case Queue of
        {queue, [], []} ->
            {error, nil};
...
-spec reverse(queue(ELY)) -> queue(ELY).
reverse(Queue) ->
...
-spec is_logically_equal(queue(EMG), queue(EMG), fun((EMG, EMG) -> boolean())) -> boolean().
...
-spec is_equal(queue(EMJ), queue(EMJ)) -> boolean().
is_equal(A, B) ->
    check_equal(
        erlang:element(3, A),
        erlang:element(2, A),
